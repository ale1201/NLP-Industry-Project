"""
Semantic injection classifier -- Multinomial Naive Bayes variant (defensive research).

Same TF-IDF features and same corpus-loading as train_semantic.py, different
classifier: Multinomial Naive Bayes instead of logistic regression. Kept as its own
file (rather than a --classifier flag) so it trains and evaluates independently and
its results file stands on its own next to train_semantic.py's and
train_semantic_svm.py's.

Usage
  python train_semantic_nb.py --train-csv ../data/deepset_train.csv --add-synthetic \
      --test-csv ../data/deepset_test.csv --out semantic_model_nb.joblib
"""
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out", default="semantic_model_nb.joblib")
